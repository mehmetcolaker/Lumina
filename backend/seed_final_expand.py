            ...
            "JavaScript", [
                {"title":"Promises in Depth","order":0,"steps":[
                    ...
                    {"title":"Fetch Users","step_type":StepType.CODE,"order":2,"xp_reward":40,
                     "content_data":{"instruction":"Write an async function `getUser(id)` that reads from the provided in-memory API data and returns the user name.\n\n```javascript\nconst users = [{ id: 1, name: 'Leanne Graham' }];\nasync function getUser(id) {\n    const user = users.find((u) => u.id === id);\n    return user.name;\n}\n```","starter_code":"const users = [{ id: 1, name: 'Leanne Graham' }];\n\nasync function getUser(id) {\n    const user = users.find((u) => u.id === id);\n    return user.name;\n}\n\ngetUser(1).then(console.log);","solution":"const users = [{ id: 1, name: 'Leanne Graham' }];\nasync function getUser(id) {\n    const user = users.find((u) => u.id === id);\n    return user.name;\n}\ngetUser(1).then(console.log);","compare_mode":"contains","expected_output":"Leanne Graham","hints":["Use the provided users array", "Return the matching user's name"]}},
                ]},
